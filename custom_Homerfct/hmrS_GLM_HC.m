% SYNTAX:
% [yavg, yavgstd, tHRF, nTrials, ynew, yresid, ysum2, beta, R, hmrstats] = hmrS_GLM_HC(dcRuns, stimRuns, probe, mlActRuns, AauxRuns, tIncAutoRuns, rcMapRuns, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector)
%
% UI NAME:
% GLM_HC_HRF_Drift_SS
%
% DESCRIPTION:
% This script is an updated version of the hmrS_GLM by Helena Cockx.
% The script estimates HRF at the session level. It concatenates run level
...
% Similar to the run level GLM, it estimates the HRF with options to specify the
...
% Updates by Helena Cockx:
% - flagNuisanceRMethod = 4: short separation regression with all the short
% channels
% - only use channels that are active in all the runs (instead of active
% channels of the first run only)
% - check Variance Inflation Factor (see: 
% https://nl.mathworks.com/matlabcentral/fileexchange/60551-vif-x)
%
%
% INPUTS:
...
%            4. if performed with all the short separation channels
...
%
% USAGE OPTIONS:
% GLM_HC_HRF_Drift_SS_Concentration: [dcAvg, dcAvgStd, nTrials, dcNew, dcResid, dcSum2, beta, R, hmrstats] = hmrS_GLM(dcRuns, stimRuns, probe, mlActRuns, AauxRuns, tIncAutoRuns, rcMapRuns, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector)
%
% PARAMETERS:
% trange: [-2.0, 20.0]
% glmSolveMethod: 1
...
function [data_yavg, data_yavgstd, nTrials, data_ynew, data_yresid, data_ysum2, beta_blks, yR_blks, hmrstats] = ...
    hmrS_GLM_HC(dcRuns, stimRuns, probe, mlActRuns, AauxRuns, tIncAutoRuns, rcMapRuns, trange, glmSolveMethod, idxBasis, paramsBasis, rhoSD_ssThresh, flagNuisanceRMethod, driftOrder, c_vector)
...
% only select channels that were active in all the runs (addition by H.C.)
allmlActRuns = zeros(length(mlActRuns{1}{1}),length(mlActRuns));
for i=1:length(mlActRuns)
  allmlActRuns(:,i) = mlActRuns{i}{1};
end
mlActAuto{1} = [sum(allmlActRuns,2)==length(mlActRuns)];
fprintf('Excluded %d channels from analysis \n', sum(~mlActAuto{1})/2)
% #### end ####
...
for iBlk=1:length(data_y)
    ...
    fprintf('... of which %d short channel(s) \n', sum(rhoSD<=rhoSD_ssThresh & mlAct(lst)==0))
    ...
    switch flagNuisanceRMethod
        case {0,1,2,4,5} % short separation
            for iConc=1:2
    ...
    for conc=1:2 %only HbO and HbR
        ...
        for iSS = 1:length(mlSSlst)
            ...
            elseif flagNuisanceRMethod == 4 | flagNuisanceRMethod == 5
                lstML = lstMLtmp(find(mlAct(lstMLtmp)==1));
                Ass = y(:,1:2,lstSS); % of both O2Hb and HHb (see Santosa, 2020)
                Ass = permute(Ass,[1 3 2]);
                Ass = cat(2, Ass(:,:,1), Ass(:,:,2)); % use O2Hb and HHb as seperate columns
                if flagNuisanceRMethod ==5
                  % pca of short channels
                  [coeff,score,latent,tsquared,explained] = pca(Ass);
                  display(explained)
                  Ass = score(:,4); % first 4 components
                end
                At = [A(:,:,conc) Ass];
            ...
            if ~isempty(lstML)
                ...
                % addition by H.C.: check multicollinearity with Variance
                % Inflation Factor
                R0 = corrcoef(At);
                V = diag(inv(R0))'; % see: https://nl.mathworks.com/matlabcentral/fileexchange/60551-vif-x
                if conc==1
                  for c = lstCond
                    fprintf('VIF for task regressor = %.2f \n', V(c))
                  end
                end
